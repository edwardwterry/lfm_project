...
class Env():
    ...
    def move(self, block, action):
        ...
        if not self.on_grid(next_coord): # move would take you over the edge
            self.block_pos[block] = (self.block_pos[block][0] + self.action_map['NO_SEL'][0], self.block_pos[block][1] + self.action_map['NO_SEL'][1])
...
if __name__ == "__main__":
    ...
    action_size = len(env.action_map) + 1 # adds I_TERM
